...
- M-x keep-lines will delete lines not matching the pattern (i.e. only keep the lines which match).
...
- C-u M-x occur will show only the portion of each line which specifically matched the pattern (which is what the quoted part of your question sounds most like).
